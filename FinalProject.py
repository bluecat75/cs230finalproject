...
URL: https://share.streamlit.io/bluecat75/cs230finalproject/main/FinalProject.py
...
price = auto["price"][(auto['year']== float(year_option))& (auto['manufacturer'] == str(brands_option))].tolist()
...
def bar_chart_1():
    ...
    plt.bar(model,price,label = "Each Model")
    plt.xlabel('Model')
    plt.ylabel('Price')
    plt.xticks(model)
    plt.xticks(rotation = 90)
    plt.title(f'All models in {year_option} {brands_option} ')
    return plt
def bar_chart_2():
...
def scatter():
    ...
    ax.set_title("Price vs. Odometer for Entire Dataset")
    ax.set_xlabel("price")
